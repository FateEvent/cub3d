...
void	doing_some_math(t_ray *ray, int i)
{
	...
	sprite = &ray->sprite;
	sprite->sprite.x = sprite->sprites[sprite->sprite_order[i]].x - ray->camera.pos.x;
	sprite->sprite.y = sprite->sprites[sprite->sprite_order[i]].y - ray->camera.pos.y;
	sprite->inv_det = 1.0 / (ray->camera.plane.x * ray->camera.dir.y - ray->camera.dir.x
			* ray->camera.plane.y);
	sprite->transform.x = sprite->inv_det * (ray->camera.dir.y * sprite->sprite.x
			- ray->camera.dir.x * sprite->sprite.y);
	sprite->transform.y = sprite->inv_det * (-ray->camera.plane.y * sprite->sprite.x
			+ ray->camera.plane.x * sprite->sprite.y);
