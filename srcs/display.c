...
/*   Updated: 2022/09/22 17:32:22 by faventur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_map(t_program *data, t_image *pixie)
{
	...
	bg.texture = mlx_load_xpm42("images/wall.xpm42");
	bg.img = mlx_texture_to_image(data->mlx, &bg.texture->texture);
	i[0] = 0;
	while (i[0] * bg.texture->texture.height < size.y && data->map[i[0]])
	{
		i[1] = 0;
		while (i[1] * bg.texture->texture.width < size.x
			&& data->map[i[0]][i[1]] != '\n')
		{
			c = data->map[i[0]][i[1]];
