...
/*   Updated: 2022/10/25 15:53:59 by albaur           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_mouse_input_child(t_var var, t_ray *ray, t_data *data, double x)
{
	if (x > data->mouse_x)
	{
		...
		ray->plane.y = var.old_plane_x * ray->mpsin + ray->plane.y * ray->mpcos;
		data->refresh = 1;
	}
	data->mouse_x = x;
}
...
void	ft_mouse_input(double x, double y, void *param)
{
	...
	if (x < data->mouse_x)
	{
		...
		ray->plane.y = var.old_plane_x * ray->mnsin + ray->plane.y * ray->mncos;
		data->refresh = 1;
	}
